        ...
        {upcomingMeetings.map(meeting => {
          ...
          return (
            <div key={meeting.id} className="p-4 bg-surface-container-high rounded-xl border border-outline-variant/10 hover:border-primary/30 transition-colors group">
              ...
              <p className="text-on-surface-variant text-body-sm truncate mb-3">{meeting.clientName || (typeof meeting.client === 'string' ? meeting.client : meeting.clientUser?.fullName) || 'Client'} • {meeting.provider === 'google-meet' ? 'Google Meet' : (meeting.provider || meeting.type || 'Google Meet')}</p>
