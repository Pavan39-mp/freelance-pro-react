...
import { Plus, Folder, CheckSquare, Calendar, FileText, Bot } from 'lucide-react';
    ...
    <div className="relative" ref={menuRef}>
      ...
      {isOpen && (
        <div className="absolute top-full  -right-2 sm:right-0 mt-2 w-56 bg-surface-container-high border border-outline-variant/20 rounded-2xl shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="py-2">
            <button onClick={() => handleAction(user?.role === 'client' ? '/client/projects' : '/freelancer/projects')} className="w-full text-left px-4 py-2 text-body-sm text-on-surface hover:bg-surface-variant/50 transition-colors flex items-center gap-3">
            ...
            <button onClick={() => handleAction()} className="w-full text-left px-4 py-2 text-body-sm text-on-surface hover:bg-surface-variant/50 transition-colors flex items-center gap-3">
      ...
      {isMeetingOpen && <ScheduleMeetingForm onClose={() => setIsMeetingOpen(false)} />}
      {isNoteOpen && <CreateNoteForm onClose={() => setIsNoteOpen(false)} />}
    </div>
  );
};
